...
export default function TopHUD({ player, rankLabel, currentRound, maxRounds, targetPoints, activeRules = [] }) {
    ...
    return (
        <div className="z-20 relative w-full flex flex-col items-center">
            <div className="hud-container w-full">
            ...
            <AnimatePresence>
                {activeRules.length > 0 && (
                    <motion.div
                        ...
                        exit={{ opacity: 0, y: -10 }}
                        className="mt-4 space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar w-full px-2"
                        style={{
                            display: 'flex',
                    ...
                    >
                        {activeRules.map((rule) => (
                            <motion.div
                                ...
                                className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-2 relative overflow-hidden group shadow-lg w-full"
                                style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    gap: '0.75rem',
